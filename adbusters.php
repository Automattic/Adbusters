 ...
 *
 * @phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound -- Legacy generic `wpcom_vip_` prefix in use
 * @phpcs:disable WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound -- Legacy generic `wpcom_vip_` prefix in use
 */
...
/**
 * Get array of Adbuster templates.
 *
...
function wpcom_vip_maybe_load_ad_busters() {
	...
	$ad_busters = array_map(
		function( $ad_file ) use ( $path ) {
			return "{$path}{$ad_file}";
		},
		$ad_busters
	);
	...
	$request = wp_parse_url( filter_input( INPUT_SERVER, 'REQUEST_URI', FILTER_SANITIZE_STRING ), PHP_URL_PATH );
